...
def main():
    ...
    parser.add_argument("--features", default="data/shared/dino")
    parser.add_argument("--caches", default="data/v1/cache")
    ...
    for index, name in enumerate(scenes, 1):
        ...
        if args.force or not feature.exists():
            command = [sys.executable, str(script_dir / "extract_dino_features.py"), str(scene), "--output", str(feature)]
            if args.device: command += ["--device", args.device]
            subprocess.run(command, check=True)
        else: print(f"  reuse {feature}")
        if args.force or not cache.exists():
            subprocess.run([sys.executable, str(script_dir / "prepare_scene.py"), "--final-data", str(scene / "final_data.pkl"), "--dino-features", str(feature), "--output", str(cache)], check=True)
