#!/usr/bin/env python3
"""Train V2-B one-step dynamics with particle-selected plateau stopping."""
...
MODEL_KEYS = [
    ...
    "x0", "neighbour_indices", "neighbour_mask", "rest_edge_lengths",
]
...
def run_epoch(model, loader, device, particle_beta, edge_vector_weight, edge_length_weight, optimizer=None):
    ...
    with torch.enable_grad() if training else torch.no_grad():
        for raw in loader:
            ...
            losses = one_step_loss(
                prediction, batch, model.spec, particle_beta, edge_vector_weight, edge_length_weight,
            )
            if training:
    ...
    metrics.update({
        "weighted_edge_vector": edge_vector_weight * metrics["edge_vector"],
        "weighted_edge_length": edge_length_weight * metrics["edge_length"],
...
def main() -> None:
    ...
    parser.add_argument("--output", default="v2/runs/one_step")
    ...
    parser.add_argument("--variant", choices=("fused", "grid_only", "particle_only"), default="fused")
    ...
    for epoch in range(1, args.epochs + 1):
        train = run_epoch(model, train_loader, device, args.particle_beta, args.edge_vector_weight, args.edge_length_weight, optimizer)
        val = run_epoch(model, val_loader, device, args.particle_beta, args.edge_vector_weight, args.edge_length_weight)
