...
from mpm_dino_v4.v42_dino_audit import dino_learnability_audit


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Audit DINO-feature versus deformation-field similarity"
    )
    parser.add_argument("--dataset", default="v41/dataset")
    parser.add_argument("--manifest", default="v41/manifests/v41_uid_splits.json")
    parser.add_argument("--output", default="v42/run/dino_learnability_audit")
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--nearest-k", type=int, default=3)
    args = parser.parse_args()
    manifest = json.loads(Path(args.manifest).read_text())
    report = dino_learnability_audit(
